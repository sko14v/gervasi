...
export function Skeleton({ className, style }: SkeletonProps) {
  return (
    <div
      className={cn(
        'animate-shimmer rounded-md bg-slate-800/40',
        className
      )}
      style={style}
    />
...
export function SkeletonLoader({ type = 'card', count = 1, className }: SkeletonLoaderProps) {
  const renderSkeleton = () => {
    switch (type) {
        ...
        return (
          <div className="space-y-3">
            {Array.from({ length: count }).map((_, i) => (
              <div key={i} className="flex items-center gap-3 rounded-lg border border-slate-800/40 bg-slate-900/40 p-3">
      ...
      case 'table':
        return (
          <div className="w-full space-y-4 rounded-xl border border-slate-800 bg-slate-900/40 p-4">
